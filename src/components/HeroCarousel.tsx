      ...
      <div className="relative aspect-[16/10] sm:aspect-[7/5] overflow-hidden" onMouseEnter={stop} onMouseLeave={start}>
        {slides.map((s, i) => (
          <div key={s.id} className="absolute inset-0 transition-opacity" style={{ opacity: i === slide ? 1 : 0, transitionDuration: "600ms" }}>
            ...
            <div className="absolute inset-x-0 bottom-0 px-4 pb-4 pt-8 sm:px-[22px] sm:pb-[22px]">
              <h3 className="text-[16px] sm:text-[19px] lg:text-[21px] font-bold text-white text-balance leading-snug">{s.title}</h3>
            </div>
          </div>
        ))}
        {slides.length > 1 && (
